...
if __name__ == "__main__":
    ...
    # vs2008 환경에서 clean하기 위한 프로젝트 파일명과 빌드 옵션을 map으로 만들어 준다.
    OMCBuildProject = {'{0}\\MainFullVersion\\MainFullVersion.sln'.format(dof_workspaces_path):'Release'
        , '{0}\\ModulesCommon\\ModulesCommon.sln'.format(dof_workspaces_path):'Release' }

    AnycatcherBuildProject = {'{0}\\MainFullVersion\\AnyCatcher.sln'.format(dof_workspaces_path):'Release_AnyCatcher' }

    BTVBuildProject = {'{0}\\OpenManager\\Modules\\Custom\\BTV\\BTV_full.sln'.format(dof_source_root): 'Release_BTV'}
    ...
    installshield = InstallerWrapper.Installer(ismList, '5.0')
